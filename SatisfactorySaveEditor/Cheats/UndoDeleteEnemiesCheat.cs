            ...
            foreach (SaveObjectModel animalSpawner in animalSpawners.DescendantSelfViewModel)
            {
                ...
                animalSpawner.FindField("mSpawnData", (ArrayPropertyViewModel arrayProperty) =>
                {
                    foreach (StructPropertyViewModel elem in arrayProperty.Elements)
                    {
                        ...
                        // Patch for un-undo the spawn location
                        if (!probablyEdited && ((Vector)((StructProperty)((DynamicStructDataViewModel)elem.StructData).Fields[0].Model).Data).Data.Z < -14200f)
                            ((Vector)((StructProperty)((DynamicStructDataViewModel)elem.StructData).Fields[0].Model).Data).Data.Z -= offset;
                        ...
                        ((IntPropertyViewModel)((DynamicStructDataViewModel)elem.StructData).Fields[3]).Value = (int)-1;
                    }
                });
            }
